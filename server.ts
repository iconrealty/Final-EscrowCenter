    ...
    try {
      ...
      if (!ai) {
        return res.status(503).json({
          success: false,
          error:
            "GEMINI_API_KEY is not configured on the server environment. Please ensure the API key is provided.",
        });
      }

      // Clean base64 string and extract actual MIME type if data URL header was sent
...
`;
      ...
      // Valid current Gemini models with fallback cascade
        ...
        while (attempts < maxAttemptsForModel) {
          ...
          try {
            ...
            if (actualMimeType.startsWith("text/")) {
              ...
              // Pass inline binary (PDF or Image)
              parts.push({
                inlineData: {
                  data: base64Clean,
            ...
            console.warn(`Model ${modelName} attempt ${attempts} encountered error:`, lastErrorMessage);

            // If 503 or 429, wait 1.2s before retry
            if (lastErrorMessage.includes("503") || lastErrorMessage.includes("429") || lastErrorMessage.includes("high demand")) {
              await new Promise((resolve) => setTimeout(resolve, 1200));
            } else {
              break; // Switch to next model immediately for other errors
            }
          }
        }
      ...
      return res.status(422).json({
        ...
        error:
          lastErrorMessage
            ...
            : "Could not extract valid transaction fields from this document. Please ensure the document is a California RPA or MLS sheet.",
      });
    } catch (error: any) {
