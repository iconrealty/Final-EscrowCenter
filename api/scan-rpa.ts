  ...
  try {
    ...
    if (!ai) {
      return res.status(422).json({
        success: false,
        error:
          "GEMINI_API_KEY is not configured on the server. Please ensure your GEMINI_API_KEY environment variable is added to Vercel/Hosting settings.",
      });
    }
...
`;
    ...
    for (const modelName of candidateModels) {
      try {
        ...
        console.warn(`Serverless model ${modelName} error:`, lastErrorMessage);
      }
    }

    let userFriendlyError = "Could not extract valid transaction fields from document.";
    if (isQuotaError) {
      userFriendlyError =
      ...
      userFriendlyError = `Gemini scanning error: ${lastErrorMessage}`;
    }
